...
export default function AuthForm({initialMode='login'}:{initialMode?:Mode}){
 const [mode,setMode]=useState<Mode>(initialMode),[busy,setBusy]=useState(false),[message,setMessage]=useState(''),[error,setError]=useState(false),[pendingEmail,setPendingEmail]=useState('');
 ...
 if(mode==='register'){const {data,error:err}=await db.auth.signUp({email,password,options:{data:{full_name:String(form.get('name'))},emailRedirectTo:window.location.origin+'/account'}});if(err)throw err;
  if(data.session){window.location.assign('/account');return;}
 ...
 else{const {error:err}=await db.auth.signInWithPassword({email,password});if(err){if(/not confirmed/i.test(err.message))setPendingEmail(email);throw err;}window.location.assign('/account');}
