...
export const ownImagePath=z.string().regex(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/[A-Za-z0-9._-]{1,100}$/,'Invalid image');
export const avatarSchema=z.object({path:ownImagePath.nullable()});
export const applicationSchema=z.object({kind:z.enum(['Driver','Courier','Merchant','Fleet operator']),city:z.enum(cities),phone,details:z.string().trim().min(10).max(1000),photoPath:ownImagePath.nullable().optional()});
...
export const productSchema=z.object({id:z.uuid().nullable(),name:z.string().trim().min(2).max(100),category:z.enum(['Food','Grocery','Accessories']),description:z.string().max(1000),price:z.number().int().min(1).max(1000000),stock:z.number().int().min(0).max(100000),active:z.boolean(),imagePath:z.string().regex(/^[A-Za-z0-9._-]{1,100}$/).or(z.literal('')).nullable().optional()});
